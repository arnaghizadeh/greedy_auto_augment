...
def create_cutout_mask(img_height, img_width, num_channels, size):
  ...
  mask = np.ones((img_height, img_width, num_channels),dtype='uint8')
  zeros = np.zeros((mask_height, mask_width, num_channels),dtype='uint8')
...
def _posterize_impl(pil_img, level):
  ...
  return ImageOps.posterize(pil_img.convert('RGB'), 4 - level)
...
def _solarize_impl(pil_img, level):
  ...
  return ImageOps.solarize(pil_img.convert('RGB'), 256 - level)
